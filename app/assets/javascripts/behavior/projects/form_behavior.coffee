namespace "Article.FormBehavior", (exports) ->
  ...
  callback = (data) ->
    $(document.getElementById('project_type_fields')).show()
    type_field = document.getElementById('project_type_id')
  ...
  behavior_on_type_click = (e) ->
    e.preventDefault()
    field = target_of_event e
    block = document.getElementById('project_new_type_fields')
    #todo toggle()
  ...
  send_new_type_request = (e) ->
    e.preventDefault()
    type_name = $(document.getElementById('project_type_name')).val()
    category_id = get_select_value(document.getElementById('project_category_id'))
    section = 'project'
  ...
  new_type_callback = (data) ->
    ...
    if !data.error
      ...
      field = document.getElementById('project_type_plus_btn')
      $(field).removeClass('unactive')


  exports.initialize = () ->
    field = document.getElementById('project_category_id')
    bind_event field, 'change', behavior_on_category_select
    #add type behavior
    field = document.getElementById('project_type_plus_btn')
    bind_event field, 'click', behavior_on_type_click

    btn = document.getElementById('project_add_type')
