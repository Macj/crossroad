namespace "Event.FormBehavior", (exports) ->
  ...
  callback = (data) ->
    $(document.getElementById('event_type_fields')).show()
    type_field = document.getElementById('event_type_id')
  ...
  behavior_on_type_click = (e) ->
    e.preventDefault()
    field = target_of_event e
    block = document.getElementById('event_new_type_fields')
    #todo toggle()
  ...
  send_new_type_request = (e) ->
    e.preventDefault()
    type_name = $(document.getElementById('event_type_name')).val()
    category_id = get_select_value(document.getElementById('event_category_id'))
    section = 'event'
  ...
  new_type_callback = (data) ->
    ...
    if !data.error
      ...
      field = document.getElementById('event_type_plus_btn')
      $(field).removeClass('unactive')


  exports.initialize = () ->
    field = document.getElementById('event_category_id')
    bind_event field, 'change', behavior_on_category_select
    #add type behavior
    field = document.getElementById('event_type_plus_btn')
    bind_event field, 'click', behavior_on_type_click

    btn = document.getElementById('event_add_type')
